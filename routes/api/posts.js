      ...
      .then(profile => {
        Post.findById(req.params.id)
          .then(post => {
            ...
            post.likes.unshift({ user: req.iser.id });
    ...
    Profile.findOne({ user: req.user.id })
      .then(profile => {
        Post.findById(req.params.id)
          .then(post => {
            if (post.likes.filter(like => like.user.toString() === req.user.id).length === 0) {
              return res.status(400).json({ disliked: 'You have not yet liked this post' });
            }
    ...
    (req, res) => {
      ...
      Post.findById(req.params.id)
        .then(post => {
          ...
          return res.stats(404).json({ commentnotexists: 'Comment does not exist' });
        }
